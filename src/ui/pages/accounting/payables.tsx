    ...
    <div className="space-y-5 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Tổng công nợ phải trả</span>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Đã chi thanh toán</span>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Còn phải trả (Dư nợ)</span>
            <div className="text-xl font-bold text-amber-700 font-mono mt-1">
              ${metrics.remaining.toLocaleString()}
            </div>
            <span className="text-[11px] text-amber-700 font-medium mt-0.5 block">Cần chuẩn bị ngân quỹ</span>
          </div>
        ...
        <div className="bg-white p-4.5 rounded-2xl border border-slate-200/90 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Đề nghị chi chờ duyệt</span>
            <div className="text-xl font-bold text-orange-700 font-mono mt-1">
              {metrics.pendingCount} phiếu
            </div>
            <span className="text-[11px] text-orange-700 font-medium mt-0.5 block">Chờ phê duyệt chi / Chờ chi</span>
          </div>
      ...
      <div className="bg-white p-3.5 rounded-2xl border border-slate-200/90 shadow-xs flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <SegmentedControl
          ...
          options={[
            ...
            { id: 'ĐÃ CHI', label: 'Đã chi đủ', count: data.filter(r => r.status === 'ĐÃ CHI').length, activeClass: 'bg-white text-emerald-700 shadow-2xs font-bold', activeBadge: 'bg-emerald-100 text-emerald-800' },
            { id: 'CHI MỘT PHẦN', label: 'Chi 1 phần', count: data.filter(r => r.status === 'CHI MỘT PHẦN').length, activeClass: 'bg-white text-blue-700 shadow-2xs font-bold', activeBadge: 'bg-blue-100 text-blue-800' },
            { id: 'CHỜ CHI', label: 'Chờ chi', count: data.filter(r => r.status === 'CHỜ CHI').length, activeClass: 'bg-white text-rose-700 shadow-2xs font-bold', activeBadge: 'bg-rose-100 text-rose-800' },
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredData.map(req => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3.5 font-mono font-bold text-rose-700 group-hover:text-rose-900 group-hover:underline">
